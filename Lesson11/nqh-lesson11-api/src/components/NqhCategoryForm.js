...
export default function NqhCategoryForm({onCloseForm, onCategorySubmit,renderNqhCategory}) {
    // State
    const [nqhId,setNqhId] = useState("");
    const [nqhCategoryName,setNqhCategoryName]= useState("");
    const [nqhCategoryStatus,setNqhCategoryStatus]= useState(true);

    useEffect(()=>{
        setNqhId(renderNqhCategory.nqhId);
        setNqhCategoryName(renderNqhCategory.nqhCategoryName);
        setNqhId(renderNqhCategory.nqhStatus);
    });
